...
public class GameStateManager : MonoBehaviour
{
    ...
    private void ForceAllEnemiesToIdle()
    {
        ...
        foreach (MonoBehaviour enemy in allEnemies)
        {
            ...
            // Check if enemy is dying (death animation playing)
            AnimatorStateInfo stateInfo = animator.GetCurrentAnimatorStateInfo(0);
            bool isDying = stateInfo.IsName("dead") || stateInfo.IsName("death") || stateInfo.IsName("Death");
            ...
            // FORCE play idle animation immediately with CrossFade for instant transition
            ...
            idleCount++;
            Debug.Log($"<color=yellow>Forced {enemy.name} to idle (disabled script, stopped coroutines, set kinematic)</color>");
        }
        
        Debug.Log($"<color=yellow>Forced {idleCount} enemies to idle state</color>");
    }

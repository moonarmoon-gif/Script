...
public class NovaStar : MonoBehaviour, IInstantModifiable
{
    ...
    public void Initialize(Transform player, int level, float angleOffset, float orbitExtension = 30f)
    {
        ...
        // Find and setup RadiusIndicator child and cache fade-in Y threshold
        FindRadiusIndicator();
    ...
    private IEnumerator OrbitalMovement()
    {
        while (isMoving)
        {
            if (!isPaused)
            {
                ...
                // Check if reached center (90° = top). Trigger the center burst
                // effect once when crossing the center, but do not pause
                // movement anymore.
    ...
    private void UpdateRadiusIndicatorAlpha()
    {
        ...
        // Fade RadiusIndicator based on the star's current Y position.
        // At or above radiusIndicatorFadeInY → fully visible at its BASE alpha.
        // A few units below that threshold → smoothly fade toward 0.
        float fadeRange = 5f;

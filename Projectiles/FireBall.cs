...
public class FireBall : MonoBehaviour
{
    ...
    public float MediumRange = 6f;
    public float FarRange = 9f;
    public float CloseMomentumDuration = 0f;
    public float MediumMomentumDuration = 0.1f;
    ...
    private Vector2 fireBitePreservedVelocity;
    private float fireBiteMomentumEndTime = -1f;
    private Animator cachedAnimator;

    private Collider2D sensorCollider2D;
    private static readonly Collider2D[] closeRangeOverlaps = new Collider2D[8];

    private float GetFireBiteMomentumDurationForDistance(float distance)
    {
        float closeRange = Mathf.Max(0f, CloseRange);
        float mediumRange = Mathf.Max(closeRange, MediumRange);
        float farRange = Mathf.Max(mediumRange, FarRange);

        float closeDuration = Mathf.Max(0f, CloseMomentumDuration);
        float mediumDuration = Mathf.Max(0f, MediumMomentumDuration);
        float farDuration = Mathf.Max(0f, FireBiteMomentumDuration);

        if (closeRange > 0f && distance <= closeRange)
        {
            return closeDuration;
        }

        if (mediumRange > closeRange && distance <= mediumRange)
        {
            float t = (distance - closeRange) / (mediumRange - closeRange);
            return Mathf.Lerp(closeDuration, mediumDuration, Mathf.Clamp01(t));
        }

        if (farRange > mediumRange && distance <= farRange)
        {
            float t = (distance - mediumRange) / (farRange - mediumRange);
            return Mathf.Lerp(mediumDuration, farDuration, Mathf.Clamp01(t));
        }

        return farDuration;
    }

    private bool ShouldUseFireBiteSpecial()
    {
        if (!EnableFireBite)
        {
            return false;
        ...
        if (!pre.fireBiteRolled)
        {
            pre.EnsureRolled();
        }

        return pre.fireBiteWillApply;
    }
    ...
    private void Awake()
    {
        ...
        {
            cachedAnimator.SetBool("firebite", false);
        }
        ...
        EnsureSensorColliderProxy();
    }

    private void EnsureSensorColliderProxy()
    {
        Transform sensor = transform.Find("SensorCollider");
        if (sensor == null)
        {
            return;
        }

        sensorCollider2D = sensor.GetComponent<Collider2D>();
        if (sensorCollider2D != null)
        {
            sensorCollider2D.enabled = false;
        }

        FireBallSensorCollider proxy = sensor.GetComponent<FireBallSensorCollider>();
        if (proxy == null)
        {
            proxy = sensor.gameObject.AddComponent<FireBallSensorCollider>();
        }

        proxy.Owner = this;
    }

    internal void OnSensorTriggerEnter(Collider2D other)
    {
        if (!EnableFireBite)
        {
            return;
        ...
        if (!ShouldUseFireBiteSpecial())
        {
            return;
        }

        if (fireBiteHasLockedTarget)
        {
            return;
        }
        ...
        if (cachedAnimator != null)
        {
            cachedAnimator.SetBool("firebite", true);
        }
        ...
        if (_rigidbody2D != null)
        {
            ...
            Vector2 enemyPos = other.bounds.center;
            float dist = Vector2.Distance((Vector2)transform.position, enemyPos);
            duration = GetFireBiteMomentumDurationForDistance(dist);
            if (duration > 0f)
            {
                fireBiteMomentumEndTime = GameStateManager.PauseSafeTime + duration;
            }
            else
            {
                fireBiteMomentumEndTime = -1f;
        ...
        StartCoroutine(FireBiteExecuteRoutine());
    }

    private IEnumerator FireBiteExecuteRoutine()
    {
        float delay = Mathf.Max(0f, ExecuteTimer);
        if (delay > 0f)
        {
            yield return GameStateManager.WaitForPauseSafeSeconds(delay);
        }

        if (fireBiteTarget != null && fireBiteTarget.IsAlive)
        {
            bool isBoss = IsFireBiteBossTarget(fireBiteTarget);

            Vector3 hitPoint = fireBiteTarget.transform.position;
            Vector3 hitNormal = Vector3.zero;

            if (!isBoss)
            {
                float killDamage = fireBiteTarget.MaxHealth * 1000f;

                EnemyDamagePopupScope.BeginSuppressPopups();
                try
                {
                    fireBiteTarget.TakeDamage(killDamage, hitPoint, hitNormal);
                }
                finally
                {
                ...
                if (fireBiteTarget != null && !fireBiteTarget.IsAlive && DamageNumberManager.Instance != null)
                {
                    Vector3 anchor = DamageNumberManager.Instance.GetAnchorWorldPosition(fireBiteTarget.gameObject, hitPoint);
            ...
            else
            {
                ...
                {
                    finalDamage = PlayerDamageHelper.ComputeProjectileDamage(cachedPlayerStats, fireBiteTarget.gameObject, baseForHit, gameObject);
                }
                ...
                fireBiteTarget.SetLastIncomingDamageType(damageType);

                fireBiteTarget.TakeDamage(finalDamage, hitPoint, hitNormal);

                StatusController.TryApplyBurnFromProjectile(gameObject, fireBiteTarget.gameObject, hitPoint, finalDamage);

                SlowEffect slowEffect = GetComponent<SlowEffect>();
                if (slowEffect != null)
                {
                    slowEffect.TryApplySlow(fireBiteTarget.gameObject, hitPoint);
                }

                StaticEffect staticEffect = GetComponent<StaticEffect>();
                if (staticEffect != null)
                {
                    staticEffect.TryApplyStatic(fireBiteTarget.gameObject, hitPoint);
                }
            }
        }

        float remaining = Mathf.Max(0f, FireBiteAnimationTime - ExecuteTimer);
        if (remaining > 0f)
        {
            yield return GameStateManager.WaitForPauseSafeSeconds(remaining);
        }
    ...
    private static bool IsFireBiteBossTarget(EnemyHealth enemyHealth)
    {
        if (enemyHealth == null)
        {
            return false;
        ...
        EnemyCardTag tag = enemyHealth.GetComponent<EnemyCardTag>() ?? enemyHealth.GetComponentInParent<EnemyCardTag>();
        ...
        else
        {
            ...
            if (cardSpawner != null && cardSpawner.CurrentBossEnemy != null)
            {
                boss = cardSpawner.CurrentBossEnemy;
            }
        }
    ...
    private void Update()
    {
        if (fireBiteHasLockedTarget && _rigidbody2D != null && fireBiteMomentumEndTime > 0f)
        {
            float now = GameStateManager.PauseSafeTime;
            if (now < fireBiteMomentumEndTime)
            {
                _rigidbody2D.velocity = fireBitePreservedVelocity;
            }
            else
            {
                _rigidbody2D.velocity = Vector2.zero;
        ...
        if (hasLaunched && !fireBiteHasLockedTarget && IsOutsideCameraBounds())
        {
            Destroy(gameObject);
            return;
        }
    ...
    public void Launch(Vector2 direction, Collider2D colliderToIgnore, PlayerMana playerMana = null, bool skipCooldownCheck = false)
    {
        ...
        bool fireBiteWillApply = EnableFireBite && pre.fireBiteWillApply;

        if (sensorCollider2D != null)
        {
            sensorCollider2D.enabled = fireBiteWillApply;
        }

        if (!fireBiteWillApply && cachedAnimator != null)
        {
            cachedAnimator.SetBool("firebite", false);
        }
        ...
        if (fireBiteWillApply && !fireBiteHasLockedTarget)
        {
            ...
            if (closeRange > 0f)
            {
                int count = Physics2D.OverlapCircleNonAlloc(transform.position, closeRange, closeRangeOverlaps, enemyLayer);
                for (int i = 0; i < count; i++)
                {
                    Collider2D hit = closeRangeOverlaps[i];
                    if (hit == null)
                    {
                        continue;
                    }
...
public sealed class FireBallSensorCollider : MonoBehaviour
{
    public FireBall Owner;
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        Owner.OnSensorTriggerEnter(other);
    }
}

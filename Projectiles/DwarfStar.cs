...
public class DwarfStar : MonoBehaviour, IInstantModifiable
{
    ...
    // Status effects are now handled by SlowEffect component
    // Add SlowEffect component to the prefab in Inspector
    ...
    public void Initialize(Transform player, int level, float angleOffset, float orbitExtension = 30f)
    {
        ...
        // DwarfStar: Start from bottom-RIGHT, move counterclockwise to bottom-LEFT
        // Counterclockwise means increasing angles.
    ...
    private IEnumerator OrbitalMovement()
    {
        while (isMoving)
        {
            if (!isPaused)
            {
                ...
                // DwarfStar: Move counterclockwise (increasing angles)
                ...
                float level1ExtraExtension = (currentLevel == 1) ? 30f : 0f;
                // Use LEFT-side extension for the completion angle so tilt is
    ...
    {
        isPaused = true;
        Debug.Log($"<color=yellow>DwarfStar paused at center for {centerPauseDuration}s</color>");
        ...
        isPaused = false;
        Debug.Log($"<color=green>DwarfStar resuming movement</color>");
    }
        ...
        if (currentLevel == 1)
        {
            ...
            if (mainCam != null)
            {
                ...
                while (true)
                {
                    ...
                    currentAngle += angleChange; // Counterclockwise
    ...
    private void DealDamageToEnemiesInRadius()
    {
        ...
        foreach (Collider2D hit in hits)
        {
            ...
            if (damageable != null && damageable.IsAlive)
            {
                ...
                // Tag EnemyHealth so the dwarf-star hit uses the ice damage
                // color in the central EnemyHealth damage-number pipeline.
                ...
                if (burnEffect != null)
                {
                    burnEffect.Initialize(finalDamage, ProjectileType.Ice);
    ...
    private void SetupVisualEffects()
    {
        if (trailRenderer != null)
        {
            ...
            Color trailColor = new Color(0f, 0.7f, 1f, 0.8f); // Cyan for ice
        ...
        if (ambientParticles != null)
        {
            ...
            main.startColor = new Color(0.3f, 0.8f, 1f, 0.8f); // Cyan for ice
            main.startSize = 0.2f;
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        if (playerTransform != null)
        {
            Gizmos.color = new Color(0f, 0.5f, 1f, 0.3f); // Cyan for ice
            float radius = GetCurrentLevelRadius();

...
public class ExecutionerEnemy : MonoBehaviour
{
    ...
    [Tooltip("Distance at which Executioner starts using ranged attacks")]
    ...
    [Tooltip("Idle duration after each projectile is fired (Executioner plays idle and does not move)")]
    ...
    [Tooltip("Minimum number of ranged projectiles Executioner will fire per volley before switching to melee.")]
    public int minRangedShots = 1;
    [Tooltip("How many ranged projectiles Executioner will fire before switching to melee only. 0 = unlimited.")]
    public int maxRangedShots = 1;
    [Tooltip("Cooldown duration after firing maxRangedShots before Executioner can start a new ranged volley (seconds). 0 = no global projectile cooldown.")]
    ...
    void FixedUpdate()
    {
        ...
        // Only consider ranged if:
        // - Enemy still has shots left in this volley OR maxRangedShots == 0 (unlimited)
        // - Projectile cooldown timer has expired
        // - Player is within shootingRange but OUTSIDE melee attackRange
        // - Enemy is allowed to shoot and not already mid-ranged attack
    ...
    IEnumerator RangedAttackRoutine()
    {
        ...
        // Initialize a new volley target the first time we enter ranged for this
        // volley. Allow minRangedShots to be 0 so configurations like
        // minRangedShots = 0, maxRangedShots = 1 produce a true 50/50 chance to
        // either fire one projectile or skip this volley entirely.
        if (maxRangedShots > 0 && currentRangedVolleyTarget <= 0)
        {
            int maxShotsForVolley = Mathf.Max(0, maxRangedShots);
            int minShotsForVolley = Mathf.Clamp(minRangedShots, 0, maxShotsForVolley);

            currentRangedVolleyTarget = Random.Range(minShotsForVolley, maxShotsForVolley + 1);

            // If this volley rolled 0 shots, treat it as an empty volley: apply
            // the global projectileCooldown once (if configured) and exit
            // without spawning a projectile. This avoids any reroll loops that
            // would otherwise force at least one shot.
        ...
        // Per-shot post ranged idle: stay idle (no walking) for the configured
        // duration after each projectile.
        ...
        // If this volley is complete (we reached the random volleyTarget and
        // maxRangedShots > 0), start the global projectile cooldown and reset
        // the per-volley counter.
